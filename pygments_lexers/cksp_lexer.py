...
class CKSPLexer(RegexLexer):
	...
	tokens = {
		'root': [
			...
			# f-strings (place BEFORE normal strings)
			...
			# # Operators
			# (r'(-|\+|\/|\*|\*\*|>>|<<|=|<|>|<=|>=|\#)', Operator),
			# (r'(and|or|not|xor|.and.|.or.|.not.|.xor.|mod)\b', Operator.Word),
			...
			(r'\b('+var_name+r')(\()', bygroups(Name.Function, Punctuation)),  # without "call"
			...
			# Namespace
			(r'\b(namespace)(\s+)', bygroups(Keyword.Declaration, Text), 'structname'),
			(r'\b(end\s+namespace)', Keyword.Declaration),
			...
			(r'\b(declare|delete|local|global|pers|instpers|read|const|polyphonic|ref)\b', Keyword.Declaration),
		...
		'expr': [
			...
			(r'[,;:]', Punctuation),
			(r'[\s+]', Whitespace),
			# light punctuation, but no parens/brackets
			# (r'[,;:]', Punctuation),
		],

		# 2) f-string double / single quote states
		'fstring_d': [
		...
		# 3) expression inside <...> (reuse expr_common; no push/pop on parens)
		'fexpr': [
			(r'>', Punctuation, '#pop'),
			include('expr'),
			# if you want parens inside <...>, add them here WITHOUT push/pop:
			(r'\(', Punctuation),
			(r'\)', Punctuation),
			(r'\[', Punctuation),
			(r'\]', Punctuation),
